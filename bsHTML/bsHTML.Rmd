...
此模版產生一個使用 Bootstrap 4 framework的html檔。
...
### Bootstrap 4
  ...
  * 不要直接剪貼bs4 html code在主Rmd檔內，而是另外存在外部`.html`檔再以下面方式引入——因為RMarkdown對於「內縮兩次以上」的文字就不會以html code來處理。
  
  * 以下範例來自Carousel with controls: https://getbootstrap.com/docs/4.4/components/carousel/

```{r, eval=T, echo=F}
htmltools::includeHTML("external/within_body/bsContent.html")
```
